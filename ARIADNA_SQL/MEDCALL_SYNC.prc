...
old_dialnum varchar(255):='';--дл€ логировани€ старый номер телефона
old_dialnum2 varchar(255):='';--дл€ логировани€ старый номер телефона
old_dialnum3 varchar(255):='';--дл€ логировани€ старый номер телефона
old_result varchar(255):='';--дл€ логировани€ старый статус
count_doc number:=0;--переменна€ нужна чтобы узнать мсключен ли локтор или нет 
dep_v number:=0;--id отделени€
doc_d number :=0;--id врача на отделении
res varchar(126):=''; 
v_pat_id number; 
exception_pat number:=0;--переменна€ нужна чтобы узнать мсключен ли пациент или нет 
begin
...
--отмена, если номерок сн€т
if (nvl(p_pat_call,0)=0 or p_pat_call=51422000) --(врач не принимает, дл€ закрыти€ расписани€)
  then
  marker:=1;
...
--,"dialnum"='0'--убрано по просьбе ¬адима 24,06,2015
where id=rc_call.id;
--commit;
    --запись в лог
...
---- провер€ем в любом случае, исключен ли клиент или нет
   --считаем кол-во исключенных
  select
  count(dd.keyid)
  into 
...
attr.linkid=rc.pat_id;
    ...
    if count_doc>0 or exception_pat>0
     then 
         res_medcall:='92';
       ---все равно добавл€ем запись со статусом 92
